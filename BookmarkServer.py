...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.end_headers()
            self.wfile.write('Invalid content send from the page. Check that both long uri and shortname are filled-in.'.encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Invalid URI. It wasn't possible to reach the address: {}".format(longuri).encode())
